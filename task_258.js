  ...
  function revrot(str, sz) {
      ...
      if (!str.length || str.length < sz || sz <= 0) {
          return ''
      }
      const arrSubStr = [];
      for (let i = 0; i < str.length; i += sz) {
          ...
          if (subStr.length === sz) {
              arrSubStr.push(subStr);
          }
      }

      let strReverted = ''
      for (let i = 0; i <= arrSubStr.length - 1; i++) {
          const sum = arrSubStr[i].split('').reduce((sum, num) => sum + (+num), 0);
          if (sum % 2) {
              strReverted += arrSubStr[i].slice(1) + arrSubStr[i][0]
          } else {
              strReverted += arrSubStr[i].split('').reverse().join('')
          }
      }
      return strReverted
  }